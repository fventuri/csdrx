...
template <typename T>
void SDRplaySourceMeasureDelay<T>::setPPM(double ppm)
{
...
// getters
template <typename T>
double SDRplaySourceMeasureDelay<T>::getSamplerate() const
{
    return samplerate;
}

template <typename T>
double SDRplaySourceMeasureDelay<T>::getBandwidth() const
{
...
template <typename T>
double SDRplaySourceMeasureDelay<T>::getFrequency() const
{
    return rx_channel_params->tunerParams.rfFreq.rfHz;
}

template <typename T>
const char* SDRplaySourceMeasureDelay<T>::getAntenna() const
{
...
template <typename T>
int SDRplaySourceMeasureDelay<T>::getIFGainReduction() const
{
    if (rx_channel_params->ctrlParams.agc.enable != sdrplay_api_AGC_DISABLE) {
        return 0;
    } else {
...
template <typename T>
unsigned char SDRplaySourceMeasureDelay<T>::getRFLnaState() const
{
    return rx_channel_params->tunerParams.gain.LNAstate;
}

template <typename T>
int SDRplaySourceMeasureDelay<T>::getIFType() const
{
...
template <typename T>
double SDRplaySourceMeasureDelay<T>::getPPM() const
{
    return device_params->devParams->ppm;
}

template <typename T>
bool SDRplaySourceMeasureDelay<T>::getDCOffset() const
{
    return (bool) rx_channel_params->ctrlParams.dcOffset.DCenable;
}

template <typename T>
bool SDRplaySourceMeasureDelay<T>::getIQBalance() const
{
