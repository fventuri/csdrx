...
#include "pulseaudiowritermeasuredelay.hpp"
...
template <typename T>
PulseAudioWriterMeasureDelay<T>::~PulseAudioWriterMeasureDelay() {
    int error;
...
template <typename T>
size_t PulseAudioWriterMeasureDelay<T>::writeable() {
    return buffer_size;
}

template <typename T>
T* PulseAudioWriterMeasureDelay<T>::getWritePointer() {
    return buffer;
}

template <typename T>
void PulseAudioWriterMeasureDelay<T>::advance(size_t how_much) {
...
namespace Csdrx {
    template class PulseAudioWriterMeasureDelay<short>;
}
